...
Json::Value serializeEvp1(Evp1& evp1)
{
	...
	for (uint i = 0; i < evp1.weightedIndices.size(); i++)
	{
		for (uint j = 0; j < evp1.weightedIndices[i].weights.size(); j++)
		{
			evpNode["weightedIndices"][i]["weights"][j] = 
				evp1.weightedIndices[i].weights[j];
		}
		for (uint j = 0; j < evp1.weightedIndices[i].weights.size(); j++)
		{
			evpNode["weightedIndices"][i]["indices"][j] = 
				evp1.weightedIndices[i].indices[j];
		}
	}
	for (uint i = 0; i < evp1.matrices.size(); i++)
	{
		Json::Value mtx;
		for (uint j = 0; j < 16; j++)
		{
