...
Json::Value serializeBck(Bck* bck)
{
	Json::Value root;
	root["AnimLength"] = bck->animationLength;
	for (uint i = 0; i < bck->anims.size(); i++)
	{
		Json::Value jointRoot = root["Joints"][i];
		for (uint j = 0; j < bck->anims[i].scalesX.size(); j++)
		{
			root["Joints"][i]["Scales"][uint(0)][j]["Time"] = bck->anims[i].scalesX[j].time;
			root["Joints"][i]["Scales"][uint(0)][j]["Value"] = bck->anims[i].scalesX[j].value;
			root["Joints"][i]["Scales"][uint(0)][j]["Tangent"] = bck->anims[i].scalesX[j].tangent;
		}
			
		for (uint j = 0; j < bck->anims[i].scalesY.size(); j++)
		{
			root["Joints"][i]["Scales"][uint(1)][j]["Time"] = bck->anims[i].scalesY[j].time;
			root["Joints"][i]["Scales"][uint(1)][j]["Value"] = bck->anims[i].scalesY[j].value;
			root["Joints"][i]["Scales"][uint(1)][j]["Tangent"] = bck->anims[i].scalesY[j].tangent;
		}

		for (uint j = 0; j < bck->anims[i].scalesZ.size(); j++)
		{
			root["Joints"][i]["Scales"][uint(2)][j]["Time"] = bck->anims[i].scalesZ[j].time;
			root["Joints"][i]["Scales"][uint(2)][j]["Value"] = bck->anims[i].scalesZ[j].value;
			root["Joints"][i]["Scales"][uint(2)][j]["Tangent"] = bck->anims[i].scalesZ[j].tangent;
		}

		for (uint j = 0; j < bck->anims[i].rotationsX.size(); j++)
		{
			root["Joints"][i]["Rotations"][uint(0)][j]["Time"] = bck->anims[i].rotationsX[j].time;
			root["Joints"][i]["Rotations"][uint(0)][j]["Value"] = bck->anims[i].rotationsX[j].value;
			root["Joints"][i]["Rotations"][uint(0)][j]["Tangent"] = bck->anims[i].rotationsX[j].tangent;
		}
		
		for (uint j = 0; j < bck->anims[i].rotationsY.size(); j++)
		{	
			root["Joints"][i]["Rotations"][uint(1)][j]["Time"] = bck->anims[i].rotationsY[j].time;
			root["Joints"][i]["Rotations"][uint(1)][j]["Value"] = bck->anims[i].rotationsY[j].value;
			root["Joints"][i]["Rotations"][uint(1)][j]["Tangent"] = bck->anims[i].rotationsY[j].tangent;
		}
		
		for (uint j = 0; j < bck->anims[i].rotationsZ.size(); j++)
		{
			root["Joints"][i]["Rotations"][uint(2)][j]["Time"] = bck->anims[i].rotationsZ[j].time;
			root["Joints"][i]["Rotations"][uint(2)][j]["Value"] = bck->anims[i].rotationsZ[j].value;
			root["Joints"][i]["Rotations"][uint(2)][j]["Tangent"] = bck->anims[i].rotationsZ[j].tangent;
		} 

		for (uint j = 0; j < bck->anims[i].translationsX.size(); j++)
		{
			root["Joints"][i]["Translations"][uint(0)][j]["Time"] = bck->anims[i].translationsX[j].time;
			root["Joints"][i]["Translations"][uint(0)][j]["Value"] = bck->anims[i].translationsX[j].value;
			root["Joints"][i]["Translations"][uint(0)][j]["Tangent"] = bck->anims[i].translationsX[j].tangent;
		}

		for (uint j = 0; j < bck->anims[i].translationsY.size(); j++)
		{	
			root["Joints"][i]["Translations"][uint(1)][j]["Time"] = bck->anims[i].translationsY[j].time;
			root["Joints"][i]["Translations"][uint(1)][j]["Value"] = bck->anims[i].translationsY[j].value;
			root["Joints"][i]["Translations"][uint(1)][j]["Tangent"] = bck->anims[i].translationsY[j].tangent;
		}

		for (uint j = 0; j < bck->anims[i].translationsZ.size(); j++)
		{
			root["Joints"][i]["Translations"][uint(2)][j]["Time"] = bck->anims[i].translationsZ[j].time;
			root["Joints"][i]["Translations"][uint(2)][j]["Value"] = bck->anims[i].translationsZ[j].value;
			root["Joints"][i]["Translations"][uint(2)][j]["Tangent"] = bck->anims[i].translationsZ[j].tangent;
		} 
	}
