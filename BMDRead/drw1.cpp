...
Json::Value serializeDrw1(Drw1& drw1)
{
	Json::Value drwNode;

	for (uint i = 0; i < drw1.isWeighted.size(); i++)
	{
		drwNode["isWeighted"][i] = (bool)drw1.isWeighted[i];
	}
	for (uint i = 0; i < drw1.data.size(); i++)
	{
		drwNode["data"][i] = drw1.data[i];
	}
