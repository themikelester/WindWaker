...
Json::Value serializeJnt1(Jnt1& jnt1)
{
	Json::Value jntNode;

	for (uint i = 0; i < jnt1.frames.size(); i++)
	{
		Frame f = jnt1.frames[i];
		jntNode["frames"][i]["scale"]["x"] = f.sx;
		jntNode["frames"][i]["scale"]["y"] = f.sy;
		jntNode["frames"][i]["scale"]["z"] = f.sz;
		
		jntNode["frames"][i]["rotation"]["x"] = f.rx;
		jntNode["frames"][i]["rotation"]["y"] = f.ry;
		jntNode["frames"][i]["rotation"]["z"] = f.rz;
		
		jntNode["frames"][i]["translation"]["x"] = f.t.x();
		jntNode["frames"][i]["translation"]["y"] = f.t.y();
		jntNode["frames"][i]["translation"]["z"] = f.t.z();
		
		jntNode["frames"][i]["name"] = f.name;
	}
