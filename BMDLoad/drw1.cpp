...
void readDrw1Header(Chunk* f, bmd::Drw1Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void dumpDrw1(Chunk* f, Drw1& dst)
{
  ...
  DSeek(f, drw1Offset + h.offsetToIsWeighted, SEEK_SET);
  for(i = 0; i < h.count; ++i)
  {
    u8 v; DRead(&v, 1, 1, f);
    if(v == 0)
  ...
  DSeek(f, drw1Offset + h.offsetToData, SEEK_SET);
  for(i = 0; i < h.count; ++i)
    readWORD(f, dst.data[i]);
}

void writeDrw1Info(Chunk* f, ostream& out)
{
  out << string(50, '/') << endl
      << "//Drw1 section" << endl
      << string(50, '/') << endl << endl;
