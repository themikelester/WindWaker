                     ...
                     Chunk* f, long offset)
{
  ...
  switch(af.dataType)
  {
    case 3: //s16 fixed point
    {
      ...
      DRead(&tmp[0], 2, length/2, f);
      data.resize(length/2);
      float scale = powf(.5f, af.decimalPoint);
    ...
    case 9: //positions
    {
      if(af.componentCount == 0) //xy
      {
        ...
        for(size_t j = 0, k = 0; j < arrays.positions.size(); ++j, k += 2)
          arrays.positions[j] = vec3(data[k], data[k + 1], 0);
      }
...
void readVtx1Header(Chunk* f, bmd::Vtx1Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void readArrayFormat(Chunk* f, bmd::ArrayFormat& af)
{
  readDWORD(f, af.arrayType);
  readDWORD(f, af.componentCount);
  readDWORD(f, af.dataType);
  DRead(&af.decimalPoint, 1, 1, f);
  DRead(&af.unknown3, 1, 1, f);
...
void dumpVtx1(Chunk* f, Vtx1& dst)
{
  ...
  DSeek(f, vtx1Offset + h.arrayFormatOffset, SEEK_SET);
...
void writeVtx1Info(Chunk* f, ostream& out)
{
  out << string(50, '/') << endl
      << "//Vtx1 section" << endl
      << string(50, '/') << endl << endl;

  int vtx1Offset = Dtell(f);
