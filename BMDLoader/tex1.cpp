...
void loadAndConvertImage(Chunk* f, const bmd::TextureHeader& h, long baseOffset,
                         Image1& curr);
...
void readTex1Header(Chunk* f, bmd::Tex1Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void readTextureHeader(Chunk* f, bmd::TextureHeader& texHeader)
{
  DRead(&texHeader.format, 1, 1, f);
  DRead(&texHeader.unknown, 1, 1, f);
  readWORD(f, texHeader.width);
  readWORD(f, texHeader.height);
  DRead(&texHeader.wrapS, 1, 1, f);
  DRead(&texHeader.wrapT, 1, 1, f);
  DRead(&texHeader.unknown3, 1, 1, f);
  DRead(&texHeader.paletteFormat, 1, 1, f);
  readWORD(f, texHeader.paletteNumEntries);
  readDWORD(f, texHeader.paletteOffset);
  readDWORD(f, texHeader.unknown5);
  DRead(&texHeader.minFilter, 1, 1, f);
  DRead(&texHeader.magFilter, 1, 1, f);
  readWORD(f, texHeader.unknown7);
  DRead(&texHeader.mipmapCount, 1, 1, f);
  DRead(&texHeader.unknown8, 1, 1, f);
  readWORD(f, texHeader.unknown9);
  readDWORD(f, texHeader.dataOffset);
}

void dumpTex1(Chunk* f, Tex1& dst)
{
  int tex1Offset = Dtell(f);
  ...
  DSeek(f, tex1Offset + h.textureHeaderOffset, SEEK_SET);
  size_t i;
...
//returns new format
u8 readImage(Chunk* f, int w, int h, u8 format, u8* palette, u8 paletteFormat, u8* dest)
{
  int srcBufferSize = getCompressedBufferSize(format, w, h);
  vector<u8> srcVec(srcBufferSize);
  u8* src = &srcVec[0];
  DRead(src, 1, srcBufferSize, f);
                         ...
                         Image1& curr)
{
  ...
  vector<u8> palette;
  bool hasPalette = (h.paletteNumEntries != 0); 
  if(hasPalette)
  {
    //read palette
    palette.resize(h.paletteNumEntries*2);
    DSeek(f, baseOffset + h.paletteOffset, SEEK_SET);
    DRead(&palette[0], 2, h.paletteNumEntries, f);
  }
  ...
  for(i = 0; i < h.mipmapCount; ++i)
  {
    ...
    if(h.dataOffset != 0)
      curr.format = readImage(f, wid, hyt, h.format,
		(hasPalette ? &palette[0] : NULL), 
		h.paletteFormat, curr.mipmaps[i]);
    else
    {
...
void writeTex1Info(Chunk* f, ostream& out)
{
  out << string(50, '/') << endl
      << "//Tex1 section" << endl
      << string(50, '/') << endl << endl;

  int tex1Offset = Dtell(f), i;
