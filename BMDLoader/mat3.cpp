...
void displayData(ostream& out, const string& str, Chunk* f, int offset, int size, int space = -1)
{
  long old = Dtell(f);
...
void displayTevStage(ostream& out, Chunk* f, int offset, int size)
{
  if(size%20 != 0)
  {
...
void writeMat3Data(ostream& debugOut, Chunk* f, int mat3Offset,
...
void readMat3Header(Chunk* f, bmd::Mat3Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void readMatIndirectTexturingEntry(Chunk* f, bmd::MatIndirectTexturingEntry& indEntry)
{
  int k, m;
  for(k = 0; k < 10; ++k)
...
void readMatEntry(Chunk* f, bmd::MatEntry& init, bool isMat2)
{
  int j;
  ...
  DRead(init.constColorSel, 1, 16, f);
  DRead(init.constAlphaSel, 1, 16, f);
...
void readTevStageInfo(Chunk* f, bmd::TevStageInfo& info)
{
  ...
  DRead(&info.colorIn, 1, 4, f);
  DRead(&info.colorOp, 1, 1, f);
  DRead(&info.colorBias, 1, 1, f);
  DRead(&info.colorScale, 1, 1, f);
  DRead(&info.colorClamp, 1, 1, f);
  DRead(&info.colorRegId, 1, 1, f);
  
  DRead(&info.alphaIn, 1, 4, f);
  DRead(&info.alphaOp, 1, 1, f);
  DRead(&info.alphaBias, 1, 1, f);
  DRead(&info.alphaScale, 1, 1, f);
  DRead(&info.alphaClamp, 1, 1, f);
  DRead(&info.alphaRegId, 1, 1, f);
  
  DRead(&info.unk2, 1, 1, f);
}
...
void dumpMat3(Chunk* f, Mat3& dst)
{
  ...
  DSeek(f, mat3Offset + h.offsets[1], SEEK_SET);
  u16 maxIndex = 0;
  ...
  //offset[4] (cull mode)
  DSeek(f, mat3Offset + h.offsets[4], SEEK_SET);
  ...
  //offset[6] (numChans)
  DSeek(f, mat3Offset + h.offsets[6], SEEK_SET);
  dst.numChans.resize(lengths[6]);
  DRead(&dst.numChans[0], 1, lengths[6], f);

  //offset[7] (colorChanInfo)
  DSeek(f, mat3Offset + h.offsets[7], SEEK_SET);
  ...
  //offset[0] (MatEntries)
  DSeek(f, mat3Offset + h.offsets[0], SEEK_SET);
  ...
  DSeek(f, mat3Offset + h.offsets[10], SEEK_SET);
  dst.texGenCounts.resize(lengths[10]);
  DRead(&dst.texGenCounts[0], 1, dst.texGenCounts.size(), f);

  //offsets[11] (texGens)
  DSeek(f, mat3Offset + h.offsets[11], SEEK_SET);
  dst.texGenInfos.resize(lengths[11]/4);
  for(i = 0; i < dst.texGenInfos.size(); ++i)
  {
    bmd::TexGenInfo info;
    DRead(&info.texGenType, 1, 1, f);
    DRead(&info.texGenSrc, 1, 1, f);
    DRead(&info.matrix, 1, 1, f);
    DRead(&info.pad, 1, 1, f);
  ...
  DSeek(f, mat3Offset + h.offsets[17], SEEK_SET);
  dst.colorS10.resize(lengths[17]/(4*2));
  for(i = 0; i < dst.colorS10.size(); ++i)
  {
    s16 col[4]; DRead(col, 2, 4, f);
  ...
  //offsets[18] (color3)
  DSeek(f, mat3Offset + h.offsets[18], SEEK_SET);
  ...
  //offset[19] (tevCounts)
  DSeek(f, mat3Offset + h.offsets[19], SEEK_SET);
  dst.tevCounts.resize(lengths[19]);
  DRead(&dst.tevCounts[0], 1, dst.tevCounts.size(), f);

  //offset[20] (TevStageInfos)
  DSeek(f, mat3Offset + h.offsets[20], SEEK_SET);
  ...
  DSeek(f, mat3Offset + h.offsets[21], SEEK_SET);
  dst.tevSwapModeInfos.resize(lengths[21]/4);
  for(i = 0; i < dst.tevSwapModeInfos.size(); ++i)
  {
    bmd::TevSwapModeInfo info;
    DRead(&info.rasSel, 1, 1, f);
    DRead(&info.texSel, 1, 1, f);
    DRead(info.pad, 1, 2, f);
  ...
  DSeek(f, mat3Offset + h.offsets[22], SEEK_SET);
  ...
  DSeek(f, mat3Offset + h.offsets[24], SEEK_SET);
  dst.alphaCompares.resize(lengths[24]/8);
  for(i = 0; i < dst.alphaCompares.size(); ++i)
  {
    bmd::AlphaCompare info;
    DRead(&info.comp0, 1, 1, f);
    DRead(&info.ref0, 1, 1, f);
    DRead(&info.alphaOp, 1, 1, f);
    DRead(&info.comp1, 1, 1, f);
    DRead(&info.ref1, 1, 1, f);
    DRead(info.pad, 1, 3, f);
  ...
  //offset[25] (blendInfo)
  DSeek(f, mat3Offset + h.offsets[25], SEEK_SET);
  dst.blendInfos.resize(lengths[25]/4);
  for(i = 0; i < dst.blendInfos.size(); ++i)
  {
    bmd::BlendInfo info;
    DRead(&info.blendMode, 1, 1, f);
    DRead(&info.srcFactor, 1, 1, f);
    DRead(&info.dstFactor, 1, 1, f);
    DRead(&info.logicOp, 1, 1, f);
  ...
  //offset[26] (z mode)
  DSeek(f, mat3Offset + h.offsets[26], SEEK_SET);
  dst.zModes.resize(lengths[26]/4);
  for(i = 0; i < dst.zModes.size(); ++i)
  {
    bmd::ZModeInfo zInfo;
    DRead(&zInfo.enable, 1, 1, f);
    DRead(&zInfo.func, 1, 1, f);
    DRead(&zInfo.updateEnable, 1, 1, f);
    DRead(&zInfo.pad, 1, 1, f);
...
void writeMat3Info(Chunk* f, ostream& out)
{
  int mat3Offset = Dtell(f);
  size_t i;
  ...
  for(i = 0; i < h.count; ++i)
  {
    u16 bla; DRead(&bla, 2, 1, f); toWORD(bla);
    maxIndex = max(maxIndex, bla);
