...
void readInf1Header(Chunk* f, bmd::Inf1Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void readInf1Entry(Chunk* f, bmd::Inf1Entry& e)
{
  readWORD(f, e.type);
  readWORD(f, e.index);
}
...
void dumpInf1(Chunk* f, Inf1& dst)
{
  ...
  //read scene graph
  DSeek(f, inf1Offset + h.offsetToEntries, SEEK_SET);
...
void writeInf1Info(Chunk* f, ostream& out)
{
  out << string(50, '/') << endl
