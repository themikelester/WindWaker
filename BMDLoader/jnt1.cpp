...
void readJnt1Header(Chunk* f, bmd::Jnt1Header& h)
{
  DRead(h.tag, 1, 4, f);
...
void readJnt1Entry(Chunk* f, bmd::JntEntry& e)
{
  readWORD(f, e.unknown);
  DRead(&e.unknown3, 1, 1, f);
  DRead(&e.pad, 1, 1, f);
...
void dumpJnt1(Chunk* f, Jnt1& dst)
{
  ...
  for(size_t i = 0; i < h.count; ++i)
  {
    ...
    f.t = vec3(e.tx, e.ty, e.tz);
    if(i < stringtable.size()) //should always be true
      f.name = stringtable[i];

    f.unknown = e.unknown;
    f.unknown3 = e.unknown3;
    f.bbMin = vec3(e.bbMin[0], e.bbMin[1], e.bbMin[2]);
    f.bbMax = vec3(e.bbMax[0], e.bbMax[1], e.bbMax[2]);
  }
}

void writeJnt1Info(Chunk* f, ostream& out)
{
  out << string(50, '/') << endl
      << "//Jnt1 section" << endl
      << string(50, '/') << endl << endl;

  int jnt1Offset = Dtell(f), i;
