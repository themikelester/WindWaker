#pragma once

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <common.h>

struct Chunk {
	Chunk(int psize) : size(psize)
	{
		base = (char*)malloc(size);
		curr = base;
		end  = base + size;
	}

	~Chunk() { free(base); }

	char* base;
	char* curr;
	char* end;
	int	  size;
};

inline int Dtell(FILE* f)
{
	return ftell(f);
}

inline int Dtell(Chunk* chnk)
{
	return chnk->curr - chnk->base;
}

inline char Dgetc(FILE* f)
{
	return fgetc(f);
}

inline char Dgetc(Chunk* chnk)
{
	return *(chnk->curr++);
}

inline int DEOD(FILE* f)
{
	return feof(f);
}

inline int DEOD(Chunk* chnk)
{
	return chnk->curr >= chnk->end;
}

inline void DRead(void* buf, int stride, int count, FILE* f)
{
	fread(buf, stride, count, f);
}

inline void DRead(void* buf, int stride, int count, Chunk* chnk)
{
	size_t s = count * stride;
	memcpy(buf, chnk->curr, s);
	chnk->curr += s;
}

inline void DSeek(FILE *f, int pos, int origin)
{
	fseek(f, pos, origin);
}

inline void DSeek(Chunk* chnk, int pos, int origin)
{
	switch(origin)
	{
	case SEEK_SET: chnk->curr = chnk->base + pos; break;
	case SEEK_CUR: chnk->curr += pos; break;
	case SEEK_END: chnk->curr = chnk->end + pos; break;
	}
}

inline void toWORD(u16& w)
{
#ifndef GC_BIG_ENDIAN
  u8 w1 = w & 0xFF;
  u8 w2 = w >> 8;
  w = (w1 << 8) | w2;
#endif
}

inline void toSHORT(s16& w)
{
  toWORD(*(u16*)&w);
}

inline void toDWORD(u32& d)
{
#ifndef GC_BIG_ENDIAN
  u8 w1 = d & 0xFF;
  u8 w2 = (d >> 8) & 0xFF;
  u8 w3 = (d >> 16) & 0xFF;
  u8 w4 = d >> 24;
  d = (w1 << 24) | (w2 << 16) | (w3 << 8) | w4;
#endif
}

inline void toFLOAT(f32& f)
{ toDWORD(*(u32*)&f); }


inline u16 aWORD(u16 w)
{
  toWORD(w); return w;
}

inline s16 aSHORT(s16 s)
{
  return aWORD(*(u16*)&s);
}

inline u32 aDWORD(u32 d)
{
  toDWORD(d); return d;
}

inline f32 aFLOAT(f32 f)
{
  toFLOAT(f); return f;
}


inline u16 memWORD(const u8* where)
{
  return (where[0] << 8) | where[1];
}

inline u32 memDWORD(const u8* where)
{
  return (where[0] << 24) | (where[1] << 16) | (where[2] << 8) | where[3];
}

template <typename T>
inline void readWORD(T* f, u16& v)
{
  DRead(&v, 2, 1, f);
  toWORD(v);
}

template <typename T>
inline void readSHORT(T* f, s16& v)
{
  DRead(&v, 2, 1, f);
  toSHORT(v);
}

template <typename T>
inline void readDWORD(T* f, u32& v)
{
  DRead(&v, 4, 1, f);
  toDWORD(v);
}

template <typename T>
inline void readFLOAT(T* f, f32& v)
{
  DRead(&v, 4, 1, f);
  toFLOAT(v);
}



inline void toWORD_le(u16& w)
{
#ifdef GC_BIG_ENDIAN
  u8 w1 = w & 0xFF;
  u8 w2 = w >> 8;
  w = (w1 << 8) | w2;
#endif
}

inline void toSHORT_le(s16& w)
{
  toWORD_le(*(u16*)&w);
}

inline void toDWORD_le(u32& d)
{
#ifdef GC_BIG_ENDIAN
  u8 w1 = d & 0xFF;
  u8 w2 = (d >> 8) & 0xFF;
  u8 w3 = (d >> 16) & 0xFF;
  u8 w4 = d >> 24;
  d = (w1 << 24) | (w2 << 16) | (w3 << 8) | w4;
#endif
}

inline void toFLOAT_le(f32& f)
{ toDWORD_le(*(u32*)&f); }


inline u16 aWORD_le(u16 w)
{
  toWORD_le(w); return w;
}

inline s16 aSHORT_le(s16 s)
{
  return aWORD_le(*(u16*)&s);
}

inline u32 aDWORD_le(u32 d)
{
  toDWORD_le(d); return d;
}

inline f32 aFLOAT_le(f32 f)
{
  toFLOAT_le(f); return f;
}

inline u16 memWORD_le(const u8* where)
{
  return where[0] | (where[1] << 8);
}

inline u32 memDWORD_le(const u8* where)
{
  return where[0] | (where[1] << 8) | (where[2] << 16) | (where[3] << 24);
}

inline void writeWORD_le(FILE* f, u16 v)
{
  toWORD_le(v);
  fwrite(&v, 2, 1, f);
}

inline void writeSHORT_le(FILE* f, s16 v)
{
  toSHORT_le(v);
  fwrite(&v, 2, 1, f);
}

inline void writeDWORD_le(FILE* f, u32 v)
{
  toDWORD_le(v);
  fwrite(&v, 4, 1, f);
}

inline void writeFLOAT_le(FILE* f, f32 v)
{
  toFLOAT_le(v);
  fwrite(&v, 4, 1, f);
}
