...
Json::Value serializeInf1(Inf1& inf1)
{
	Json::Value infNode;

	infNode["numVertices"] = inf1.numVertices;
	for (uint i = 0; i < inf1.scenegraph.size(); i++)
	{
		Node n = inf1.scenegraph[i];
		infNode["scenegraph"][i]["type"] = n.type;
		infNode["scenegraph"][i]["index"] = n.index;
	}
